...
export default class MainMenu{
	...
	create() {
		...
		while(ys.length > 0) {
			...
			let text = this.make.text(110, 5, data.teamName + '游出' + data.totalAchievement + '米', { font: '22px', fill: '#fff'});
			person.addChild(text);
			ys.splice(i, 1);
		}


		this.bridge = this.add.sprite(0, 310, 'bridge');
		
		let grass = this.add.sprite(0, this.game.height - 148, 'grass');

		this.tBtn = this.add.button(this.game.width / 2 - 320, this.game.height - 190, 'team_btn', this.click2, this);
		this.sfBtn = this.add.button(this.game.width / 2 + 20, this.game.height - 190, 'self_btn', this.click1, this);
		// createSuccess();
	...
	setBtnGrey() {
		this.tBtn.loadTexture('team_btn_grey');
		this.sfBtn.loadTexture('self_btn_grey');
	}
